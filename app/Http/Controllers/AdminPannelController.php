...
class AdminPannelController extends Controller
{
    function home(){
        ...
        $visitors = User::where('userType', '=', 'Customer')->get();
        ...
        return view('Admin.Home', ['user' => $user, 'visitors' => $visitors, 'sales' => $sales->total, 'order' => $order]);
    }
    ...
    function OrderListChangeStatus($id)
    {
        
        $order = order::where('userName', '=', $id)->first();
    ...
    public function seeOrder($id){
        ...
        $cart = Cart::where('userName', '=', $id)->get();
    
        
        return view('Admin.seeOrder', ['cart' => $cart]);
